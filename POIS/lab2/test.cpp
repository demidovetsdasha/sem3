...
TEST(TheatreTest, ActorsTest) {
    ...
    DollActor actor3("Лена", "Паулинка");
    BalletDancer actor4("Анна", "Лебедь");

    EXPECT_EQ(actor1.GetName(), "Катя");
    EXPECT_EQ(actor1.GetRole(), "Рамона");
    EXPECT_EQ(actor3.GetName(), "Лена");
    EXPECT_EQ(actor3.GetRole(), "Паулинка");
    EXPECT_EQ(actor4.GetName(), "Анна");
...
TEST(TheatreTest, TheatresTest) {
    ...
    DollTheatre theatre3("Кукольный театр", "ул. Цветочная, 4", "Кировец", perfomances);
    BalletTheatre theatre4("Театр балета", "ул. Дворовая, 7", "Моринов", perfomances);

    EXPECT_EQ(theatre1.GetName(), "Большой театр");
    EXPECT_EQ(theatre3.GetName(), "Кукольный театр");
    EXPECT_EQ(theatre4.GetName(), "Театр балета");
